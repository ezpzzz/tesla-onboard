...
async function signedPost(env: Env, phase: "authorize" | "init" | "finalize", path: string, body: Uint8Array) {
  ...
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) throw new Error(`capture_${phase}_malformed_response`);
  return parsed as Record<string, unknown>;
}

function requireString(source: Record<string, unknown>, field: string, phase: string): string {
  const value = source[field];
  if (typeof value !== "string" || value.length === 0) throw new Error(`capture_${phase}_missing_${field}`);
  return value;
}

function requireFiniteNumber(source: Record<string, unknown>, field: string, phase: string): number {
  const value = source[field];
  if (typeof value !== "number" || !Number.isFinite(value)) throw new Error(`capture_${phase}_missing_${field}`);
  return value;
}

function assertAuthorizeResponse(response: Record<string, unknown>): { objectKey: string; inboundId: string; aliasRevision: number } {
  return {
    inboundId: requireString(response, "inbound_id", "authorize"),
    aliasRevision: requireFiniteNumber(response, "accepted_alias_revision", "authorize"),
    objectKey: requireString(response, "object_key", "authorize"),
  };
}

function assertCaptureResponse(response: Record<string, unknown>, phase: "init" | "finalize"): void {
  requireString(response, "id", phase);
  requireString(response, "state", phase);
}
...
export default {
  async email(message: EmailMessage, env: Env): Promise<void> {
    ...
    try {
      ...
      const normalized: NormalizedEmailManifest = normalizeParsedEmail(parsed, { from: message.from, to: message.to, sourceMessageId });
      ...
      const { objectKey, inboundId, aliasRevision } = authorized;
      ...
      const initResponse = await signedPost(env, "init", "/api/internal/turo-email/capture?phase=init", canonicalJsonBytes(init));
      assertCaptureResponse(initResponse, "init");
