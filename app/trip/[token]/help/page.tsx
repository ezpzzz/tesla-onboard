  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_360px]">
      ...
      <aside className="space-y-4">
        ...
        <Card className="p-5"><h2 className="font-semibold">Your host</h2><div className="mt-4 space-y-3 text-sm text-muted">{config.hostName ? <div>{config.hostName}</div> : null}{config.hostPhone ? <a href={`tel:${config.hostPhone}`} rel="noreferrer" className="flex items-center gap-2 text-brand"><IconPhone className="h-4 w-4" />{config.hostPhone}</a> : null}{config.supportEmail ? <a href={`mailto:${config.supportEmail}`} rel="noreferrer" className="flex items-center gap-2 text-brand"><IconMail className="h-4 w-4" />{config.supportEmail}</a> : null}</div></Card>
      </aside>
    </div>
  );
}
