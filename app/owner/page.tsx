    ...
    <div className="space-y-6">
        ...
        <Card className="overflow-hidden">
          <div className="grid lg:grid-cols-[1.12fr_0.88fr]">
            <section className="border-b border-line lg:border-b-0 lg:border-r" aria-labelledby="next-handoff-title">
              <div className="px-6 pt-7"><h2 id="next-handoff-title" className="font-semibold">Next handoff · {new Intl.DateTimeFormat("en-US", { hour: "numeric", minute: "2-digit" }).format(new Date(handoff.boundaryAt))}</h2><p className="mt-1 text-sm text-muted">{handoff.location ?? "Location in trip instructions"}</p></div>
            ...
            <section className="p-6 lg:p-8" aria-labelledby="handoff-guest-title">
      ...
      <div className="grid gap-4 lg:grid-cols-[1.15fr_0.85fr]">
        <section className="rounded-lg border border-line bg-white px-6 py-3" aria-labelledby="today-handoffs-title">
          <h2 id="today-handoffs-title" className="border-b border-line py-3 text-[17px] font-semibold">Today’s handoffs</h2>
        ...
        <section className="rounded-lg border border-line bg-white px-6 py-3" aria-labelledby="attention-title">
          <h2 id="attention-title" className="border-b border-line py-3 text-[17px] font-semibold">Needs attention · {attention.length}</h2>
      ...
      <section className="grid grid-cols-3 divide-x divide-line rounded-lg border border-line bg-white py-5 text-center">
        <div><div className="text-2xl font-semibold">{stats.tripCounts.active + stats.tripCounts.upcoming}</div><div className="mt-1 text-xs text-muted">Trips in motion</div></div>
        <div><div className="text-2xl font-semibold">{drivers.filter((driver) => driver.progress?.isDone).length}</div><div className="mt-1 text-xs text-muted">Guests ready</div></div>
        <div><div className="text-2xl font-semibold">{attention.length}</div><div className="mt-1 text-xs text-muted">Actions needed</div></div>
      </section>
