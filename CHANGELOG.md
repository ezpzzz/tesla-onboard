...
- Added roughly 85 net tests, including two coverage-audit regression e2e specs (single-shell SSR, kWh-not-dollars Energy tile).
- Telemetry thresholds -- freshness windows, the charge-session gap rule, history retention, charging-sync intervals -- are centralized in `lib/owner/telemetry-policy.ts` rather than scattered across call sites.
