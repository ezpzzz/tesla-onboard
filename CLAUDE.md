...
- **The guest-progress bridge** (`lib/progress-bridge.ts`) has two explicit modes. A valid private trip token scopes local guest state and sends a bounded, debounced progress snapshot to `/api/guest/trips/progress`; the server binds the authenticated booking email and records its own receipt timestamp so owners can track another device. Demo/non-trip browsing retains the tenant-scoped local bridge. The native `storage` event still only fires in other tabs, so same-tab demo navigation uses the focus/visibility fallback.
- **`/trip/<token>` deep links**: the private token is the durable booking capability and replaces any previously captured trip token. Guest local state is scoped by that token so separate bookings never share progress. The public invitation RPC returns only guest-safe workspace, trip, and vehicle presentation data; booking email is compared by hash and remains private.
