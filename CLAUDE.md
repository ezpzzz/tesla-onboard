...
- **`lib/owner/*`** is the owner-side equivalent of `lib/tesla.ts` + `lib/content.ts`: `types.ts` (`OwnerSnapshot`, driver/trip/session/vehicle shapes), `data-source.ts` (the `OwnerDataSource` seam; its current implementation returns honest empty collections until a persistent bookings/charging adapter exists), `derive.ts`/`alerts.ts` (pure functions computing dashboard-facing view state from the snapshot), `owner-state.ts` (`useOwnerState()`, localStorage key `rtr:owner:v1`, per-trip return checklists — mirrors `lib/store.ts`'s load/save/hook pattern exactly but is a fully separate key so it can never collide with guest state), and `vehicle-state.ts` (`useVehicleState()`, localStorage key `rtr:vehicles:v1`, the host's editable vehicle roster; fresh stores start empty and only Tesla import/manual add creates records). Load migrations remove the retired listing seed, mock-Tesla persona import, and fixture-trip checklist keys without touching real or owner-edited vehicles.
...
- **Vehicles**: `rtr:vehicles:v1` localStorage store, empty on first load; only a live Tesla import or explicit manual add creates a record. `Vehicle` is a field on `OwnerSnapshot`. `resolveVehiclePolicyPct` is the single source of truth for policy percentage — nothing else computes it independently. Removal is archive-not-delete (vehicles are hidden, never destroyed, so trip/driver history stays intact). Owner Tesla connect may make one optional, no-wake `vehicle_data?endpoints=vehicle_config` read for each of the first 10 cars to import trim/color/interior/wheels; guest Tesla sign-in remains list-only. `lib/vehicle-media.ts` maps verified model/year/trim/paint/wheel tuples to Tesla's public Design Studio compositor. Exact media must fail closed: never render a different paint, trim, generation, wheel package, or conflicting interior when a tuple is unknown. Legacy specs are normalized, but unsupported legacy generations use neutral artwork. `VehicleArtwork` must retain its failure fallback. The guest-facing flow still reads `hostConfig.car` directly and does not consult the browser-local vehicle store — publishing owner imports to renters requires a persistent listing/trip-to-vehicle backend and must not be faked with localStorage.
- **First-run setup wizard (`/owner/setup`)**: a pure step machine at `lib/owner/setup-flow.ts` (`welcome → connect → import → review → done`, structural mirror of `lib/flow.ts`), with its own localStorage state at `lib/owner/setup-state.ts` — key `rtr:owner-setup:v1`, its own key so it can never collide with `rtr:owner:v1`/`rtr:vehicles:v1`/the guest's `rtr:state:v1`. Dedupe is keyed by `teslaImportKey` (`vin ?? tesla vehicle id`, `lib/owner/import-mapping.ts`) recorded on the vehicle record itself (not only in the wizard's `importedTeslaIds` ledger), so re-running Import is idempotent even after "Start over" clears `rtr:owner-setup:v1` in any mode, and correctly offers to restore/update a previously-archived vehicle instead of creating a duplicate. `reset()` clears only this key, never `rtr:vehicles:v1`. Connecting Tesla for the wizard is a full owner-side mirror of the guest connect flow (`lib/owner/use-owner-tesla-connect.ts`, `app/api/owner/tesla/{login,me,logout}/route.ts`): separate `rtr_owner_state`/`rtr_owner_tesla` cookies, the same one-shot "read identity + vehicles once, discard tokens" posture as the guest's live mode, and its own `TESLA_OWNER_REDIRECT_URI` env var for live mode (distinct from the guest's `TESLA_REDIRECT_URI` so the two OAuth identities never share a redirect target). These owner-side OAuth routes (`app/api/owner/tesla/*`, `app/auth/owner/tesla/callback`) are included in `middleware.ts`'s matcher, so when Supabase owner auth is configured they sit behind the same login gate as the rest of `/owner` — an unauthenticated visitor cannot drive the owner Tesla OAuth flow directly. In mock mode the wizard reuses the existing consent screen (`app/auth/tesla/page.tsx`) via an additive `?mode=owner&return=/owner/setup` branch — parsed client-side, post-mount, behind an open-redirect guard (`isSafeReturnPath`, which rejects control characters and normalizes the URL before validating it) — rather than a second consent UI. The overview page (`app/owner/page.tsx`) only ever links into `/owner/setup` from its "Set up your fleet" card (or the "Fleet setup" header link); there is no auto-redirect into the wizard.
