...
`lib/store.ts` `useOnboarding()` is the single state hook — `OnboardingState` persists under `rtr:state:v1:<shop-slug>` so a guest can resume without leaking progress across tenants. `lib/tenant-storage.ts` owns key scoping and one-time migration from the former unscoped keys. Note `newToTesla` is intentionally **stable across sign-in** (it does not flip when experience changes) so the account-setup step doesn't vanish mid-walkthrough. `stepId` is the source of truth for "which step."
...
- **`lib/tenant-config.ts`** defines and runtime-validates guest-safe configuration stored under `workspace_branding.features.onlyevs`. Its `car` is a versioned materialized snapshot with `sourceVehicleId` + `sourceVehicleUpdatedAt`: `sourceVehicleId` is a random opaque UUID migrated onto each local fleet record, not its sequential `veh-*` id or VIN, so another browser cannot accidentally bind an unrelated car. Guests cannot read the private browser-local fleet, so only model/trim/year/color/shifter/wheels/interior/Tesla option codes are published. VIN, plate, notes, and operational data never enter workspace branding. `OwnerTenantProvider` resolves the authenticated user's workspace membership and every branded shop within it; `TenantConfigProvider` resolves public guest configuration by the unambiguous `?tenant=<workspace-id>~<shop-slug>` reference (legacy slug-only links still work). Existing workspace RLS policies authorize owner writes. Never place Supabase/Tesla credentials or cookie secrets in tenant JSON.
- **`/owner/setup` and `/owner/settings`** are the editing surfaces for brand/contact, guest vehicle, rental policy, and house rules. Selecting an active fleet vehicle establishes the source link and publishes its exact guest-safe snapshot. Owner sessions automatically refresh that snapshot when the linked fleet record changes; manual vehicle-field edits intentionally clear the source link so the UI cannot claim two drifting records are the same car. Saving updates the active branded shop without a redeploy and preserves unrelated `features` keys.
