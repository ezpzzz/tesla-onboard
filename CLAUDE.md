...
- **`lib/tenant-config.ts`** defines and runtime-validates guest-safe configuration stored under `workspace_branding.features.onlyevs`. `OwnerTenantProvider` resolves the authenticated user's workspace membership and every branded shop within it; `TenantConfigProvider` resolves public guest configuration by the unambiguous `?tenant=<workspace-id>~<shop-slug>` reference (legacy slug-only links still work). Existing workspace RLS policies authorize owner writes. Never place Supabase/Tesla credentials or cookie secrets in tenant JSON.
- **`/owner/setup` and `/owner/settings`** are the editing surfaces for brand/contact, guest vehicle, rental policy, and house rules. An active imported fleet vehicle can populate the guest vehicle, including exact paint/interior/wheel option data. Saving updates the active branded shop without a redeploy and preserves unrelated `features` keys.
