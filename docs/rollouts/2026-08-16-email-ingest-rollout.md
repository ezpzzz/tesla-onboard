...
- **Candidate-to-trip apply pipeline (create/update/cancel sagas, the 30-minute alert-gated destructive brake, the abort control, and the four sequential capability gates).** Approved, in-branch-plan scope per the locked eng plan, and — per the next-phase note above — now authorized to build in full, not schema-only. As of this doc's last reconciliation there is **no app-layer executor** yet — `onlyevs-worker` hardcodes every inserted candidate to `state='needs_review'` regardless of the per-integration review/auto mode toggle in `components/owner/OwnerIntegrations.tsx` (persisted via `POST /api/owner/integrations/turo-email`); `canAutoApply()` (`lib/email/capabilities.ts`) is wired but not called from any apply path. This is Review-only by design today; building the executor is the next implementation phase, gated on real `.eml` fixtures (§7 below) — richer parsing (exact reservation/timezone/vehicle/guest-delivery match) is a prerequisite for even the first rung of live mutation (Auto create, ladder rung 4 in §3).
