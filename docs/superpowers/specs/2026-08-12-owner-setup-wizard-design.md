...
2. Register the new URI with Tesla: re-run `node --env-file=.env.local scripts/tesla-setup.mjs register`, or add it via the Tesla developer dashboard if the script only registers on first run — confirm current script behavior before assuming it's idempotent for additive URIs.
3. Verify with `node --env-file=.env.local scripts/tesla-setup.mjs verify` that Tesla has both the guest and owner callback URIs on file.
