...
- **`ImportStep` empty-state gating confirmed correct.** The empty state gates on the raw `teslaProfile.vehicles.length === 0` — not a filtered "importable" count — and that's sufficient: an account with zero vehicles on the Tesla side (or no connected account at all) is the only case it needs to catch. An account whose only vehicle is already imported still has `vehicles.length === 1 > 0`, so it falls through to the picker branch instead, where `existingVehicleFor` (matching by `teslaImportKey`, see above) marks that vehicle checked-disabled with an "Already imported" badge rather than re-offering it. The empty-state guard itself never needs to know which vehicles are already imported.
